...
if ( LegalPressHelpers::is_woocommerce_active() ) {
	 ...
	 */
	add_action( 'woocommerce_before_main_content', 'legalpress_theme_wrapper_start', 10 );
	add_action( 'woocommerce_after_main_content', 'legalpress_theme_wrapper_end', 10 );

	function legalpress_theme_wrapper_start() {
		$sidebar = legalpress_get_shop_sidebar();
	...
	<div class="content-area  container">
		<div class="row">
			<main id="main" class="site-main  col-xs-12<?php echo 'left' === $sidebar ? '  col-md-9  col-md-push-3' : ''; echo 'right' === $sidebar ? '  col-md-9' : ''; ?>" role="main">
				...
				function legalpress_theme_wrapper_end() {
					$sidebar = legalpress_get_shop_sidebar();
				?>
			</main>
			<?php if ( 'none' !== $sidebar ) : ?>
	 ...
	 * @return array
	 */
	function legalpress_woo_body_class( $classes ) {
	...
	add_filter( 'body_class', 'legalpress_woo_body_class', 11 );

	// Display custom number of products per page
	function legalpress_custom_number_of_products_per_page( $cols ) {
		return absint( get_theme_mod( 'products_per_page', '9' ) );
	}
	add_filter( 'loop_shop_per_page', 'legalpress_custom_number_of_products_per_page', 20 );
	 ...
	 * @return array
	 */
	function legalpress_output_related_products_args( $args ) {
		$args['posts_per_page'] = 3;
		$args['columns']        = 3;

		return $args;
	}
	add_filter( 'woocommerce_output_related_products_args', 'legalpress_output_related_products_args' );

	/**
	 * Get the position of the sidebar for the shop pages, conditionally for the single product
	 */
	function legalpress_get_shop_sidebar() {
		if ( is_product() ) {
	 ...
	 */
	if ( ! function_exists( 'legalpress_loop_columns' ) ) {
		function legalpress_loop_columns() {
			return 3; // 3 products per row
		}
	}
	add_filter( 'loop_shop_columns', 'legalpress_loop_columns' );
