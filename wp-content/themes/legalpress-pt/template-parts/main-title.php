...
// woocommerce
if ( LegalPressHelpers::is_woocommerce_active() && is_woocommerce() ) {
	$bg_id = $shop_id;
}
...
$style_attr = LegalPressHelpers::create_background_style_attr( $style_array );

?>

<div class="main-title<?php echo 'small-title-area' === get_theme_mod( 'main_title_mode', 'big-title-area' ) ? '  main-title--small' : '' ?>" style="<?php echo esc_attr( $style_attr ); ?>">
	<div class="container">
		<?php
		$main_tag = 'h1';
		$subtitle = false;
		...
		if ( is_home() || ( is_single() && 'post' === get_post_type() ) ) {
		...
		} elseif ( LegalPressHelpers::is_woocommerce_active() && is_woocommerce() ) {
				ob_start();
			...
			$title = esc_html__( 'Search Results For' , 'legalpress-pt' ) . ' &quot;' . get_search_query() . '&quot;';
		} elseif ( is_404() ) {
			$title = esc_html__( 'Error 404' , 'legalpress-pt' );
		} else {
			$title    = get_the_title();
