...
function get_colorful_categories( $categories ) {
    if ( count( $categories ) == 0 ) {
        return;
    }

    $colorful_categories = array();
    foreach ($categories as $category) {
        $color = rl_color( $category->cat_ID );

        $html = '<a href="' . get_category_link($category->cat_ID) . '" ';
        if ( !empty( $color ) ) {
            $html .= 'style="background-color: ' . $color . '" ';
        }
        $html .= ' rel="category" >' . $category->cat_name . '</a>';

        $colorful_categories[] = $html;
    }
    return $colorful_categories;
}
